...
build_client_object({T, B0}=B, K, V) when is_binary(T), is_binary(B0), is_binary(K) orelse undefined =:= K ->
    {ok,  #riakc_obj{bucket = B, key = K, contents = [], updatevalue = V}};
